...
// Regenerate: node scripts/upgrade-modules.js imaging_module4.js
    ...
    '3d-vision': {
      ...
      sections: [
        ...
        },
        {
          ...
          content: [
            '<p>A <strong>3D vision</strong> (or geometric computer vision) is the field of recovering three-dimensional structure and motion from 2D images or video. Unlike 2D image processing, 3D vision aims to infer depth, shape, spatial layout, and camera pose — enabling machines to perceive the world in three dimensions. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>3D vision</strong> (or geometric computer vision) is the field of recovering three-dimensional structure and motion from 2D images or video. Unlike 2D image processing, 3D vision aims to infer depth, shape, spatial layout, and camera pose — enabling machines to perceive the world in three dimensions. Structure from Motion (SfM): recover 3D scene geometry and camera trajectories from unordered images Multi-View Stereo (MVS): generate dense 3D models from multiple calibrated views SLAM (Simultaneous Localization and Mapping): build a map while tracking the sensor position in real time Shape from X: infer shape from shading, texture, focus, or contours Applications span robotics, AR/VR, autonomous driving, archaeology, and digital twin creation</p>',
            '<p>You use 3d vision when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — 3D Vision

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: 3D Vision sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
...
Example values:
  ...
  Z = 800 * 0.10 / 40 = 2.0 meters`,
        ...
        },
        {
          ...
          example: {
            title: '3D Vision with Python',
            code: `import numpy as np
# 3D Vision — synthetic 16-bit slice
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Multi-View Stereo',
                'Ordered / calibrated views',
                'Dense depth map / mesh',
                'Dense'
              ],
              [
                'Depth from Stereo',
                'Stereo image pair',
                'Per-pixel disparity / depth',
                'Dense'
              ],
              [
                'SLAM',
              ...
              [
                'Photogrammetry',
                'Overlapping photos with metadata',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        },
        {
          ...
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>3D Vision</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
...
plt.title("Bill by day — 3D Vision")
plt.suptitle("")
plt.show()`,
    ...
    'stereo-vision': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>A <strong>stereo vision</strong> is the process of inferring depth from two images captured by cameras at different positions, mimicking human binocular perception. By finding correspondences between the two views and measuring their horizontal displacement (disparity), we can compute per-pixel depth using known camera geometry. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>stereo vision</strong> is the process of inferring depth from two images captured by cameras at different positions, mimicking human binocular perception. By finding correspondences between the two views and measuring their horizontal displacement (disparity), we can compute per-pixel depth using known camera geometry. Epipolar geometry constrains the search for corresponding points to a single line, dramatically reducing complexity Rectification warps stereo pairs so that epipolar lines are horizontal and aligned, simplifying matching Disparity is the horizontal pixel shift of a point between left and right images Depth is computed from disparity using triangulation with known focal length and baseline Modern stereo systems use block matching, semi-global matching (SGM), or learned deep stereo networks</p>',
            '<p>You use stereo vision when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Stereo Vision

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Stereo Vision sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
For a point at Z = 10 m:
  d = 700 * 0.12 / 10 = 8.4 px`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Stereo Vision with Python',
            code: `import numpy as np
# Stereo Vision — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'Block Matching (BM)',
                'SSD/SAD on fixed windows',
                'Very fast',
                'Noisy at depth edges'
              ],
              [
                'Semi-Global Matching (SGM)',
                'Path-wise dynamic programming',
                'Fast',
                'Smooth, preserves edges'
              ],
              [
                'Graph Cuts',
              ...
              [
                'Deep Stereo (PSMNet)',
                'CNN feature + 3D cost volume',
                'GPU real-time',
                'State-of-the-art accuracy'
              ],
              [
                'Census Transform',
                'Robust binary pattern matching',
                'Fast',
                'Good on textureless regions'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Stereo Vision</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Stereo Vision")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'depth-estimation': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>A <strong>depth estimation</strong> is the task of predicting the distance from the camera to every point in a scene. It can be performed with a single image (monocular depth), a stereo pair, multiple views, or active sensors. Monocular depth estimation has advanced dramatically with deep learning, using learned scene priors to infer plausible geometry from a single photograph. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>depth estimation</strong> is the task of predicting the distance from the camera to every point in a scene. It can be performed with a single image (monocular depth), a stereo pair, multiple views, or active sensors. Monocular depth estimation has advanced dramatically with deep learning, using learned scene priors to infer plausible geometry from a single photograph. Monocular depth: single image → relative or metric depth map; relies on learned size, perspective, and occlusion cues Stereo depth: two calibrated views → metric depth via triangulation and disparity matching Multi-view depth: N calibrated views → dense depth via plane-sweep or MVS fusion Active depth: structured light, time-of-flight (ToF), or LiDAR directly measure distance Depth is represented as a depth map (Z per pixel), a point cloud, or a mesh surface</p>',
            '<p>You use depth estimation when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Depth Estimation

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Depth Estimation sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Example: Zmin=0.5m, Zmax=10m, N_planes=256`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Depth Estimation with Python',
            code: `import numpy as np
# Depth Estimation — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
          ...
          table: {
            ...
            rows: [
              ...
              [
                ...
                '~0.5–50 m',
                'Medium (cm at 5m)',
                'Textureless, occlusions'
              ],
              [
                'Structured Light',
                '0.1–5 m',
                'Very high (mm)',
                'Outdoor/sunlight fail'
              ],
              [
                'Time-of-Flight (ToF)',
                '0.1–10 m',
              ...
              [
                ...
                '1–200 m',
                'High (cm)',
                'Sparse (unless solid-state)'
              ],
              [
                'Photogrammetry',
                'Any',
                'High (cm–mm)',
                'Requires many overlapping images'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Depth Estimation</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Depth Estimation")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    photogrammetry: {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>A <strong>photogrammetry</strong> is the science of making precise measurements from photographs. By capturing overlapping images of an object or terrain from multiple positions, photogrammetry reconstructs accurate 3D coordinates, orthomosaics, digital elevation models (DEMs), and textured meshes. It bridges the gap between photography and surveying. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>photogrammetry</strong> is the science of making precise measurements from photographs. By capturing overlapping images of an object or terrain from multiple positions, photogrammetry reconstructs accurate 3D coordinates, orthomosaics, digital elevation models (DEMs), and textured meshes. It bridges the gap between photography and surveying. Aerial / drone photogrammetry: map terrain, vegetation, and infrastructure from UAV imagery Close-range photogrammetry: model small objects, sculptures, and industrial parts with sub-millimeter precision Bundle adjustment: jointly optimize 3D point positions and camera poses to minimize reprojection error Scale is established from known ground control points (GCPs), GPS/RTK data, or scale bars Output products include dense point clouds, textured meshes, orthophotos, and contour maps</p>',
            '<p>You use photogrammetry when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Photogrammetry

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Photogrammetry sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Example result after BA:
  ...
  → sub-pixel accuracy achieved`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Photogrammetry with Python',
            code: `import numpy as np
# Photogrammetry — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'Camera distance',
                '50–500 m (drone) to 1–10 km (aircraft)',
                '0.1–10 m'
              ],
              [
                'Subject',
                'Terrain, buildings, forests, agriculture',
                'Artifacts, sculptures, machine parts'
              ],
              [
                'Scale control',
                'GPS/RTK ground control points',
                'Scale bars, calibration rigs'
              ],
              [
                'Accuracy',
                '1–5 cm absolute',
                '0.01–0.1 mm relative'
              ],
              [
                'Output',
                'Orthomosaic, DEM, contour lines',
                'Textured mesh, CAD model'
              ],
              [
                'Regulation',
                'Airspace, flight permits',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Photogrammetry</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Photogrammetry")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'point-clouds': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>A <strong>point cloud</strong> is a set of 3D coordinates (X, Y, Z) that sample the surface of objects or environments. Point clouds may also carry additional attributes: color (RGB), intensity, normal vectors, or classification labels. They are the raw output of LiDAR scanners, depth cameras, and multi-view stereo pipelines, and serve as the starting point for many 3D analysis tasks. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>point cloud</strong> is a set of 3D coordinates (X, Y, Z) that sample the surface of objects or environments. Point clouds may also carry additional attributes: color (RGB), intensity, normal vectors, or classification labels. They are the raw output of LiDAR scanners, depth cameras, and multi-view stereo pipelines, and serve as the starting point for many 3D analysis tasks. Raw sensor output: LiDAR returns a point cloud of range measurements; stereo depth produces a dense colored point cloud Unordered nature: unlike images, point clouds have no grid structure, requiring specialized neural architectures (PointNet, PointCNN) Direct 3D representation: no lossy conversion to meshes or voxels; geometry is preserved at native resolution Processing includes filtering (outlier removal, downsampling), registration (aligning multiple scans), and segmentation Common formats: LAS/LAZ (LiDAR), PLY, PCD, E57, and XYZ</p>',
            '<p>You use point clouds when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Point Clouds

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Point Clouds sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
9. Iterate until convergence (|error_change| < ε)`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Point Clouds with Python',
            code: `import numpy as np
# Point Clouds — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'LiDAR',
                'Sparse (64–1024 beams)',
                '1–3 cm',
                'Autonomous driving, mapping'
              ],
              [
                'Depth camera (ToF)',
                'Dense (VGA–HD)',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Point Clouds</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Point Clouds")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'mesh-processing': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>A <strong>mesh processing</strong> encompasses the algorithms that create, refine, simplify, repair, and analyze polygonal meshes — typically triangle meshes — that represent surfaces in 3D. Meshes are the dominant representation for graphics, simulation, and manufacturing because they explicitly encode connectivity, enabling efficient rendering, collision detection, and finite-element analysis. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>mesh processing</strong> encompasses the algorithms that create, refine, simplify, repair, and analyze polygonal meshes — typically triangle meshes — that represent surfaces in 3D. Meshes are the dominant representation for graphics, simulation, and manufacturing because they explicitly encode connectivity, enabling efficient rendering, collision detection, and finite-element analysis. Reconstruction: converting point clouds or implicit fields into watertight meshes (Poisson reconstruction, marching cubes, Delaunay triangulation) Simplification: reducing polygon count while preserving shape (edge collapse, quadric error metrics) Smoothing and denoising: removing acquisition noise while preserving sharp features (Laplacian smoothing, bilateral mesh filtering) Repair: filling holes, removing self-intersections, and ensuring manifold topology Parameterization and remeshing: improving triangle quality and creating UV maps for texturing</p>',
            '<p>You use mesh processing when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Mesh Processing

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Mesh Processing sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Example:
  ...
  vi_new = vi + 0.5 * L(vi) = (1.0, 2.0, 0.533)`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Mesh Processing with Python',
            code: `import numpy as np
# Mesh Processing — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'Marching Cubes',
                'Implicit field / SDF grid',
                'Triangle mesh',
                'Watertight if grid is closed'
              ],
              [
                'Poisson Reconstruction',
                'Oriented point cloud + normals',
                'Watertight mesh',
              ...
              [
                'Alpha Shapes',
                'Point cloud + radius α',
                'Polygonal mesh',
                'Controls level of detail'
              ],
              [
                'Ball Pivoting',
                'Point cloud + ball radius',
                'Triangle mesh',
                'Interpolates input points'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Mesh Processing</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Mesh Processing")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'volume-rendering': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>A <strong>volume rendering</strong> is a technique for displaying 3D datasets — typically scalar fields defined on a regular grid (voxels) — as 2D images without first extracting an intermediate surface. It is essential for visualizing phenomena that have no well-defined surfaces: medical scans (CT, MRI), weather simulations, fluid dynamics, and seismic data. Volume rendering composites color and opacity along rays cast through the volume. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>volume rendering</strong> is a technique for displaying 3D datasets — typically scalar fields defined on a regular grid (voxels) — as 2D images without first extracting an intermediate surface. It is essential for visualizing phenomena that have no well-defined surfaces: medical scans (CT, MRI), weather simulations, fluid dynamics, and seismic data. Volume rendering composites color and opacity along rays cast through the volume. Direct volume rendering (DVR): ray casting through voxels, accumulating color and opacity according to a transfer function Transfer function: maps voxel intensity (scalar value) to color (RGB) and opacity (α), revealing different tissues or materials Maximum intensity projection (MIP): displays only the brightest voxel along each ray, common in angiography Isosurface rendering: extracts a surface at a specific threshold using marching cubes (not true volume rendering, but often compared) Acceleration structures: empty space skipping, early ray termination, and GPU texture slicing make real-time volume rendering feasible</p>',
            '<p>You use volume rendering when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Volume Rendering

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Volume Rendering sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Final pixel color = C_out`,
        ...
        },
        {
          ...
          example: {
            ...
            code: `import numpy as np
# Volume Rendering — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
          ...
          table: {
            ...
            rows: [
              [
                ...
                'Translucent, layered',
                'CT/MRI anatomy education',
                'Requires good transfer function'
              ],
              [
                'Maximum Intensity Projection',
                'Brightest-voxel-only',
                'Angiography, vessel tracking',
                'No depth cue, loss of context'
              ],
              [
                'Isosurface (Marching Cubes)',
                'Hard surface',
                'Bone, segmented organs',
                'Misses fuzzy boundaries'
              ],
              [
                'Slice Rendering',
                '2D cross-section',
                'Measurement, diagnosis',
                'No 3D spatial context'
              ],
              [
                'Multi-Planar Reconstruction',
                'Orthogonal slices',
                'Surgical planning',
                'Mental 3D reconstruction needed'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Volume Rendering</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Volume Rendering")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'medical-imaging': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>A <strong>medical imaging</strong> is the discipline of creating visual representations of the interior of the body for clinical analysis and medical intervention. It spans physics (how energy interacts with tissue), engineering (sensor and system design), and computer science (image reconstruction, visualization, and AI-assisted diagnosis). Modalities differ in the physical principle used to probe anatomy and the clinical questions they answer. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, A <strong>medical imaging</strong> is the discipline of creating visual representations of the interior of the body for clinical analysis and medical intervention. It spans physics (how energy interacts with tissue), engineering (sensor and system design), and computer science (image reconstruction, visualization, and AI-assisted diagnosis). Modalities differ in the physical principle used to probe anatomy and the clinical questions they answer. X-ray / CT: uses ionizing radiation; excellent for bone, lung, and trauma; CT provides 3D cross-sections MRI: uses magnetic fields and radio waves; superior soft-tissue contrast; no ionizing radiation Ultrasound: uses acoustic waves; real-time, portable, safe; ideal for cardiac, prenatal, and point-of-care imaging PET / SPECT: uses radioactive tracers; reveals metabolic activity; combined with CT or MRI for anatomical context Optical / Endoscopy: visible light cameras for surface and cavity inspection; combined with AI for polyp detection</p>',
            '<p>You use medical imaging when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Medical Imaging

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Medical Imaging sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
In practice (discrete):
  ...
  4. Sum all back-projections to reconstruct slice`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Medical Imaging with Python',
            code: `import numpy as np
# Medical Imaging — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
          ...
          table: {
            ...
            rows: [
              ...
              [
                'MRI',
                'Magnetic resonance',
                'Brain, spine, soft tissue',
                'Metal implants contraindicated; loud'
              ],
              [
                'Ultrasound',
                'Acoustic waves',
                'Cardiac, fetal, guiding needles',
                'Operator dependent; limited depth'
              ],
              [
                'PET',
                'Positron emission',
                'Cancer staging, metabolism',
                'Radiotracer injection; expensive'
              ],
              [
                'Mammography',
                'Low-dose X-ray',
                'Breast cancer screening',
                'Compression discomfort; density limits'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Medical Imaging</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Medical Imaging")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
