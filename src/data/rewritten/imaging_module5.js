...
Example pixel values:
  ...
  If T = 10 → M = 1 (motion detected)`,
        ...
        },
        {
          ...
          table: {
            headers: ['Aspect', 'Image Processing', 'Video Processing'],
            rows: [
              ['Input', 'Single static image', 'Sequence of frames over time'],
              ['Data volume', 'One 2D array (H × W)', 'Multiple 2D arrays (T × H × W)'],
              ['Redundancy', 'Only spatial', 'Spatial + temporal'],
              ['Key operations', 'Filtering, transforms, segmentation', 'Motion estimation, tracking, temporal filtering'],
              ['Complexity', 'Lower; single frame analysis', 'Higher; must maintain temporal consistency'],
              ['Compression', 'JPEG, PNG', 'H.264, H.265 (exploit temporal redundancy)']
            ]
          }
        },
        {
...
Lucas-Kanade solves for (u, v) using a window of pixels:
  ...
  [ΣIₓIᵧ  ΣIᵧ²  ] [v] = [-ΣIᵧIₜ]`,
        ...
        },
        {
          ...
          table: {
            headers: ['Aspect', 'Sparse Optical Flow', 'Dense Optical Flow'],
            rows: [
              ['Tracked points', 'Selected interest points (corners, features)', 'Every pixel in the image'],
              ['Method', 'Lucas-Kanade on keypoints', 'Horn-Schunck, Farneback, deep learning'],
              ['Speed', 'Fast (few points)', 'Slow (many computations)'],
              ['Accuracy', 'Accurate at tracked points', 'Can be noisy in uniform regions'],
              ['Use case', 'Object tracking, motion analysis', 'Video segmentation, frame interpolation'],
              ['Output', 'Few motion vectors', 'Full motion field']
            ]
          }
        },
        {
...
Example:
  ...
  L_temp penalizes any label flip at that location.`,
        ...
        },
        {
          ...
          table: {
            headers: ['Aspect', 'Image Segmentation', 'Video Segmentation'],
            rows: [
              ['Input', 'Single frame', 'Sequence of frames'],
              ['Temporal awareness', 'None', 'Must maintain label consistency across time'],
              ['Occlusion handling', 'Not applicable', 'Must handle objects appearing/disappearing'],
              ['Output flickering', 'Not a concern', 'Major issue — masks must be stable'],
              ['Model architecture', 'CNN (U-Net, DeepLab)', 'CNN + RNN, 3D conv, or memory networks'],
              ['Annotation cost', 'Lower (one frame)', 'Higher (must label across frames)']
            ]
          }
        },
        {
...
Example scores:
  ...
  Fused (average): 0.775 → classified as "running"`,
        ...
        },
        {
          ...
          table: {
            headers: ['Aspect', '2D CNN + LSTM', '3D CNN (C3D, I3D)'],
            rows: [
              ['Spatial features', 'Extracted per frame', 'Extracted jointly across space and time'],
              ['Temporal modeling', 'LSTM / GRU over frame features', 'Built into 3D convolutions'],
              ['Parameter count', 'Lower (2D conv + RNN)', 'Higher (3D kernels)'],
              ['Training data', 'Less demanding', 'Requires more data due to more parameters'],
              ['Speed', 'Faster inference', 'Slower but captures spatiotemporal patterns directly'],
              ['Representative', 'Two-Stream + LSTM', 'I3D, SlowFast networks']
            ]
          }
        },
        {
...
Example:
  ...
  L_adv = 0.8 (realistic textures)`,
        ...
        },
        {
          ...
          table: {
            headers: ['Aspect', 'Video Prediction', 'Video Generation'],
            rows: [
              ['Input', 'Past video frames', 'Text, noise, or single image'],
              ['Output', 'Future frames', 'Complete video sequence'],
              ['Goal', 'Continue existing dynamics', 'Create new content from scratch'],
              ['Coherence', 'Must match past motion', 'Must invent consistent motion'],
              ['Architecture', 'ConvLSTM, PredRNN', 'Video diffusion models, VAEs, GANs'],
              ['Evaluation', 'PSNR, SSIM vs ground truth', 'FVD (Fréchet Video Distance), human preference']
            ]
          }
        },
        {
