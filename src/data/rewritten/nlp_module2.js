...
// Regenerate: node scripts/upgrade-modules.js nlp_module2.js
    ...
    'sentiment-analysis': {
      ...
      sections: [
        ...
        },
        {
          ...
          content: [
            '<p>Sentiment analysis (or opinion mining) is the NLP task of identifying the emotional tone behind a piece of text. It classifies text as positive, negative, or neutral, and can also detect finer emotions like anger, joy, or sadness. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Sentiment analysis (or opinion mining) is the NLP task of identifying the emotional tone behind a piece of text. It classifies text as positive, negative, or neutral, and can also detect finer emotions like anger, joy, or sadness. Binary classification: positive vs negative sentiment Multi-class: positive, negative, neutral, or emotion labels Aspect-based: detects sentiment toward specific product features Intensity scoring: rates how strong the sentiment is (e.g., +0.8 vs +0.3)</p>',
            '<p>You use sentiment analysis when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Sentiment Analysis

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Sentiment Analysis sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
...
print("Sentiment Analysis — TF-IDF shape:", X.shape)
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Best for',
                'Quick prototypes, domains with stable vocab',
                'Medium-scale classification',
                'Production, nuanced text'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        },
        {
          ...
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Sentiment Analysis</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
...
plt.title("Bill by day — Sentiment Analysis")
plt.suptitle("")
plt.show()`,
    ...
    tfidf: {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>TF-IDF is a statistical measure that evaluates how important a word is to a document within a collection (corpus). It balances how often a word appears in a document against how common it is across all documents. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, TF-IDF is a statistical measure that evaluates how important a word is to a document within a collection (corpus). It balances how often a word appears in a document against how common it is across all documents. TF (Term Frequency): how often a word appears in a document IDF (Inverse Document Frequency): how rare the word is across the corpus TF-IDF = TF × IDF: words frequent in a document but rare in the corpus score highest Used for: information retrieval, text classification, keyword extraction, document similarity</p>',
            '<p>You use tf-idf when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — TF-IDF

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: TF-IDF sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Doc3: "The bird flew high" (5 words)
  ...
  TF-IDF("the") = 0.2 × 0 = 0 ← STOPWORD`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'TF-IDF with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("TF-IDF — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
          ...
          table: {
            ...
            rows: [
              [
                ...
                'Simple word count',
                'TF × log(N/DF)',
                'Optimized TF-IDF with length normalization'
              ],
              [
                'Normalization',
                'None',
                'None',
                'Document length compensated'
              ],
              [
                'Stopwords',
                'Dominate scores',
                'Downweighted to near zero',
                'Downweighted'
              ],
              [
                'Rare words',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>TF-IDF</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
    ...
    'word2vec-nlp': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>Word2Vec is a shallow neural network model that learns dense vector representations (embeddings) of words from large text corpora. Words with similar contexts end up with similar vectors, capturing semantic and syntactic relationships. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Word2Vec is a shallow neural network model that learns dense vector representations (embeddings) of words from large text corpora. Words with similar contexts end up with similar vectors, capturing semantic and syntactic relationships. Two architectures: CBOW (predicts target from context) and Skip-gram (predicts context from target) Trained via sliding context windows over raw text Output: fixed-size dense vectors (typically 100–300 dimensions) Enables vector arithmetic: king − man + woman ≈ queen</p>',
            '<p>You use word2vec for nlp when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Word2Vec for NLP

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Word2Vec for NLP sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Result after training:
  ...
  v_fox · v_table ≈ 0.02 (low similarity)`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Word2Vec for NLP with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Word2Vec for NLP — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Training speed',
                'Faster (averages context vectors)',
                'Slower (more updates)'
              ],
              [
                'Rare words',
              ...
              [
                ...
                'May overfit rare words'
              ],
              [
                'Best for',
                'Frequent words, syntactic tasks',
                'Rare words, semantic tasks'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Word2Vec for NLP</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Word2Vec for NLP")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'sequence-tagging': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>Sequence tagging is the NLP task of assigning a label to each element (token) in an input sequence. Unlike classification, which labels the whole text, sequence tagging labels every individual token. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Sequence tagging is the NLP task of assigning a label to each element (token) in an input sequence. Unlike classification, which labels the whole text, sequence tagging labels every individual token. POS tagging: labels each word with its grammatical role (noun, verb, adjective) NER: labels tokens as person, organization, location, or other entity types Chunking: groups words into syntactic phrases (noun phrases, verb phrases) BIO tagging scheme: B (begin), I (inside), O (outside) marks entity boundaries</p>',
            '<p>You use sequence tagging when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Sequence Tagging

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Sequence Tagging sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Constraint: I-PER can only follow B-PER or I-PER`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Sequence Tagging with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Sequence Tagging — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Dependencies',
                'Limited (previous tag only)',
                'Arbitrary features',
                'Bidirectional context + CRF',
                'Full context'
              ],
              [
                'Features',
                'Word + previous tag',
                'Rich handcrafted or learned',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Sequence Tagging</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Sequence Tagging")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'dependency-parsing': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>Dependency parsing analyzes the grammatical structure of a sentence by identifying directed relationships (dependencies) between words. Each word (except the root) has exactly one head word it depends on, labeled with a grammatical relation type. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Dependency parsing analyzes the grammatical structure of a sentence by identifying directed relationships (dependencies) between words. Each word (except the root) has exactly one head word it depends on, labeled with a grammatical relation type. Identifies head-modifier relationships in a sentence Labels relations: nsubj (nominal subject), dobj (direct object), amod (adjectival modifier), etc. Produces a tree structure (single root, connected, no cycles) Used for: information extraction, question answering, machine translation, relation extraction</p>',
            '<p>You use dependency parsing when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Dependency Parsing

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Dependency Parsing sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Tree structure:
 ...
 The     on  the`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Dependency Parsing with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Dependency Parsing — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Output',
                'Directed tree (arcs)',
                'Hierarchical tree (nested phrases)'
              ],
              [
                'Root',
                'Single root verb',
                'Root is the full sentence (S)'
              ],
              [
                'Relation labels',
                'Grammatical functions (nsubj, dobj)',
                'Syntactic categories (NP, VP)'
              ],
              [
                'Best for',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Dependency Parsing</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Dependency Parsing")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'constituency-parsing': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>Constituency parsing (or phrase structure parsing) breaks a sentence into nested syntactic constituents — phrases like noun phrases (NP), verb phrases (VP), and prepositional phrases (PP) — arranged in a hierarchical tree. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Constituency parsing (or phrase structure parsing) breaks a sentence into nested syntactic constituents — phrases like noun phrases (NP), verb phrases (VP), and prepositional phrases (PP) — arranged in a hierarchical tree. Organizes words into nested phrases based on grammar rules Root node represents the full sentence (S) Non-terminal nodes are syntactic categories (NP, VP, PP, ADJP) Terminal nodes are the actual words (leaves of the tree) Context-Free Grammars (CFG) formalize the rewriting rules</p>',
            '<p>You use constituency parsing when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Constituency Parsing

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Constituency Parsing sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Parse tree:
      ...
      |     |   |  /  /    The   cat sat Det N  P NP
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Constituency Parsing with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Constituency Parsing — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Training',
                'Needs grammar rules',
                'Needs annotated treebank',
                'Needs treebank + rule counts'
              ],
              [
                'Ambiguity',
                'Returns all parses',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Constituency Parsing</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Constituency Parsing")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'machine-translation': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>Machine Translation (MT) is the task of automatically converting text from one natural language (source) to another (target). Modern MT systems use deep learning, particularly encoder-decoder architectures with attention, to produce fluent and accurate translations. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Machine Translation (MT) is the task of automatically converting text from one natural language (source) to another (target). Modern MT systems use deep learning, particularly encoder-decoder architectures with attention, to produce fluent and accurate translations. Rule-based MT: uses bilingual dictionaries and hand-crafted grammar rules Statistical MT (SMT): learns translation probabilities from parallel corpora Neural MT (NMT): uses sequence-to-sequence models with attention Transformer-based MT: current state-of-the-art (Google Translate, DeepL)</p>',
            '<p>You use machine translation when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Machine Translation

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Machine Translation sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Decoder predicts:
  P("The" | <BOS>, c₁) → highest probability`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Machine Translation with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Machine Translation — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Output fluency',
                'Often rigid and unnatural',
                'Better but still choppy',
                'Fluent and natural'
              ],
              [
                'Rare words',
                'Handled by dictionary',
                'Poor (out-of-phrase-table)',
                'Poor (OOV) but subword helps'
              ],
              [
                'Word reordering',
                'Explicit rules',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Machine Translation</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Machine Translation")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'text-summarization': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>Text summarization is the task of producing a shorter version of a text while preserving its key information and meaning. It reduces reading time and helps users quickly grasp the essence of long documents. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Text summarization is the task of producing a shorter version of a text while preserving its key information and meaning. It reduces reading time and helps users quickly grasp the essence of long documents. Extractive summarization: selects and concatenates the most important sentences from the source Abstractive summarization: generates new sentences that paraphrase the source content Single-document vs multi-document summarization Query-focused: summary tailored to a specific user question</p>',
            '<p>You use text summarization when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Text Summarization

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Text Summarization sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Text Summarization — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Compression',
                'Moderate (sentence-level)',
                'High (phrase-level)'
              ],
              [
                'Complexity',
                'Lower (scoring + ranking)',
                'Higher (seq2seq, transformers)'
              ],
              [
                'Best for',
                'News, reports, factual documents',
                'Creative writing, conversational summaries'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Text Summarization</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Text Summarization")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'question-answering': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>Question Answering (QA) is the NLP task of automatically producing an answer to a natural language question, typically based on a provided context (passage, document, or knowledge base). It spans from simple fact lookup to complex multi-hop reasoning. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, Question Answering (QA) is the NLP task of automatically producing an answer to a natural language question, typically based on a provided context (passage, document, or knowledge base). It spans from simple fact lookup to complex multi-hop reasoning. Extractive QA: answer is a span of text directly from the context (SQuAD style) Abstractive QA: generates an answer in free-form natural language Open-domain QA: answers from a large corpus or the web without a given passage Multi-hop QA: requires reasoning across multiple documents or facts</p>',
            '<p>You use question answering when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Question Answering

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Question Answering sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Answer span: start=Albert (index 1), end=Einstein (index 2)
  → "Albert Einstein"`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Question Answering with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Question Answering — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'Answer source',
                'Span from provided context',
                'Generated freely',
                'Retrieved docs + generation'
              ],
              [
                'Factuality',
                'High (grounded in text)',
                'Risk of hallucination',
                'Moderate to high'
              ],
              [
                'Complexity',
                'Lower (span prediction)',
                'Higher (seq2seq)',
                'Highest (retrieval + generation)'
              ],
              [
                'Context required',
                'Single passage',
                'Can be passage or open',
                'Large corpus or web'
              ],
              [
                'Best for',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Question Answering</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Question Answering")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
