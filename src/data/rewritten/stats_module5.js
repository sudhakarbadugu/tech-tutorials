export const statsModule5Structure = {
  module5: {
    ...
    topics: [
      { id: 'time-series-stats', title: 'Time Series Analysis' },
      { id: 'survival-analysis', title: 'Survival Analysis' },
      { id: 'experimental-design', title: 'Experimental Design' },
      { id: 'multivariate-stats', title: 'Multivariate Statistics' },
      { id: 'bayesian-stats', title: 'Bayesian Statistics' },
    ]
  }
};
    ...
    'time-series-stats': {
      ...
      sections: [
        ...
        },
        {
          ...
          table: {
            headers: ['Property', 'Stationary Series', 'Non-Stationary Series'],
            rows: [
              ['Mean', 'Constant over time', 'Changes over time'],
              ['Variance', 'Constant over time', 'Changes over time'],
              ['Autocorrelation', 'Depends only on lag, not time', 'Depends on both lag and time'],
              ['Example', 'White noise, temperature residuals', 'Stock prices, GDP, population'],
              ['Modeling approach', 'ARMA, direct regression', 'Differencing, ARIMA, transformation'],
              ['Forecast behavior', 'Reverts to long-run mean', 'Follows trend; can diverge']
            ]
          }
        },
        {
    ...
    'survival-analysis': {
      ...
      sections: [
        ...
        },
        {
          ...
          table: {
            headers: ['Concept', 'Description', 'When to Use'],
            rows: [
              ['Right-censoring', 'Event not yet occurred by study end', 'Most common; ongoing subjects'],
              ['Left-censoring', 'Event occurred before observation began', 'Retrospective studies with unknown start'],
              ['Interval-censoring', 'Event known to occur within an interval', 'Periodic inspections (e.g., dental visits)'],
              ['Kaplan-Meier', 'Non-parametric survival curve', 'Visualizing survival; comparing groups with log-rank test'],
              ['Nelson-Aalen', 'Non-parametric cumulative hazard', 'When hazard interpretation is more natural than survival'],
              ['Cox PH', 'Semi-parametric regression', 'Assessing covariate effects without specifying baseline hazard'],
              ['Parametric (Weibull, Exp)', 'Fully specified distributions', 'When you need to predict exact survival times or extrapolate beyond data']
            ]
          }
        },
        {
    ...
    'experimental-design': {
      ...
      sections: [
        ...
        },
        {
          ...
          table: {
            headers: ['Design', 'Description', 'Best For'],
            rows: [
              ['Completely Randomized', 'Subjects randomly assigned to treatments', 'Homogeneous subjects; simplest analysis'],
              ['Randomized Block', 'Subjects grouped into blocks, then randomized within block', 'Known nuisance factor (e.g., gender, location)'],
              ['Latin Square', 'Two blocking factors in a square layout', 'Controlling two nuisance factors with few resources'],
              ['Factorial (2^k)', 'All combinations of k factors at two levels each', 'Screening many factors and their interactions'],
              ['Split-Plot', 'Whole-plot factors applied to large units; sub-plot factors to sub-units', 'When some factors are hard to change (e.g., oven temperature)'],
              ['Repeated Measures', 'Same subjects measured under all conditions', 'Reducing between-subject variability; studying temporal effects'],
              ['Crossover', 'Subjects switch treatments after a washout period', 'Comparing treatments while controlling individual differences']
            ]
          }
        },
        {
    ...
    'multivariate-stats': {
      ...
      sections: [
        ...
        },
        {
          ...
          table: {
            headers: ['Method', 'Goal', 'Assumptions', 'Output'],
            rows: [
              ['PCA', 'Maximize variance; reduce dimensions', 'Linear, no noise model', 'Components ordered by variance'],
              ['Factor Analysis', 'Model observed variables with latent factors', 'Unique factors + common factors', 'Factor loadings + uniquenesses'],
              ['LDA (Discriminant)', 'Maximize between-group separation', 'Multivariate normal, equal covariances', 'Discriminant functions'],
              ['MANOVA', 'Test group differences on multiple DVs', 'Multivariate normal, homogeneity of covariance', 'Wilks\' Lambda, Pillai\'s Trace'],
              ['Clustering (k-means)', 'Group observations by similarity', 'Spherical clusters, chosen k', 'Cluster assignments'],
              ['MDS', 'Preserve pairwise distances in low-D', 'Distance metric chosen', 'Coordinate representation']
            ]
          }
        },
        {
    ...
    'bayesian-stats': {
      ...
      sections: [
        ...
        },
        {
          ...
          table: {
            headers: ['Aspect', 'Bayesian', 'Frequentist'],
            rows: [
              ['Probability meaning', 'Degree of belief', 'Long-run frequency'],
              ['Parameter status', 'Random variable with a distribution', 'Fixed unknown constant'],
              ['Inference target', 'Posterior distribution P(θ|D)', 'Point estimate + sampling distribution'],
              ['Uncertainty interval', 'Credible interval: P(a < θ < b|D) = 0.95', 'Confidence interval: 95% of intervals contain θ over repeated samples'],
              ['Interpretation', '"There is a 95% probability θ is in this interval"', '"If we repeated the experiment infinitely, 95% of such intervals would contain θ"'],
              ['Prior information', 'Explicitly incorporated', 'Not used'],
              ['Small samples', 'Naturally regularized by prior', 'Can be unstable or biased'],
              ['Computation', 'Often requires MCMC or variational methods', 'Usually closed-form or asymptotic']
            ]
          }
        },
        {
