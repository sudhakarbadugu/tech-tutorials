...
// Regenerate: node scripts/upgrade-modules.js nlp_module1.js
    ...
    overview: {
      ...
      sections: [
        ...
        },
        {
          ...
          content: [
            '<p><strong>Natural Language Processing (NLP)</strong> is a branch of artificial intelligence that enables computers to understand, interpret, and generate human language. It bridges the gap between human communication and machine understanding by processing text and speech data. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Natural Language Processing (NLP)</strong> is a branch of artificial intelligence that enables computers to understand, interpret, and generate human language. It bridges the gap between human communication and machine understanding by processing text and speech data. NLP combines computational linguistics, computer science, and machine learning to analyze language It handles both written text and spoken speech through subfields like Natural Language Understanding (NLU) and Natural Language Generation (NLG) Modern NLP relies heavily on deep learning, especially transformer architectures like BERT and GPT Applications range from simple spell-checkers to sophisticated conversational AI systems</p>',
            '<p>You use overview of natural language processing when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Overview of Natural Language Processing

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Overview of Natural Language Processing sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
...
Structured Output`,
        ...
        },
        {
          ...
          example: {
            title: 'Overview of Natural Language Processing with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Overview of Natural Language Processing — TF-IDF shape:", X.shape)
          ...
          table: {
            ...
            rows: [
              [
                ...
                'Probabilistic models (n-grams, HMM)',
                'Neural networks (RNN, Transformer)'
              ],
              [
                'Data needed',
                'Minimal',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        },
        {
          ...
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Overview of Natural Language Processing</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
...
plt.title("Bill by day — Overview of Natural Language Processing")
plt.suptitle("")
plt.show()`,
    ...
    tokenization: {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Tokenization</strong> is the process of splitting raw text into smaller units called tokens. Tokens can be words, subwords, characters, or sentences. It is the first essential step in any NLP pipeline because models cannot process raw text directly — they need discrete units to operate on. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Tokenization</strong> is the process of splitting raw text into smaller units called tokens. Tokens can be words, subwords, characters, or sentences. It is the first essential step in any NLP pipeline because models cannot process raw text directly — they need discrete units to operate on. Word tokenization splits text on whitespace and punctuation: "Hello world!" → ["Hello", "world", "!"] Subword tokenization breaks rare words into smaller known pieces: "unhappiness" → ["un", "happiness"] Character tokenization splits into individual letters: "cat" → ["c", "a", "t"] Sentence tokenization splits paragraphs into sentences for document-level processing</p>',
            '<p>You use tokenization when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Tokenization

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Tokenization sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Tokenization — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Tokenization</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Tokenization")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'text-preprocessing': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Text preprocessing</strong> is the process of cleaning and transforming raw text into a consistent, usable format before tokenization and modeling. Raw text from the web, social media, or documents contains noise — HTML tags, URLs, emojis, inconsistent casing, and special characters — that degrades model performance if left unhandled. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Text preprocessing</strong> is the process of cleaning and transforming raw text into a consistent, usable format before tokenization and modeling. Raw text from the web, social media, or documents contains noise — HTML tags, URLs, emojis, inconsistent casing, and special characters — that degrades model performance if left unhandled. Lowercasing normalizes text to a single case for consistency, though case can carry meaning ("US" vs "us") Removing URLs, emails, and HTML tags eliminates non-linguistic noise common in web-scraped data Removing or normalizing special characters and emojis prevents token pollution Stopword removal and spelling correction reduce vocabulary noise and improve feature quality</p>',
            '<p>You use text preprocessing when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Text Preprocessing

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Text Preprocessing sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Step 5 — Tokenize:
  ["check", "nlp", "awesome"]`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Text Preprocessing with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Text Preprocessing — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'Lowercasing',
                'General text classification, search',
                'NER, sentiment (case matters)',
                'Reduces vocab size by ~30%'
              ],
              [
                'Remove URLs',
                'Social media, web text',
                'URL classification tasks',
                'Eliminates non-semantic tokens'
              ],
              [
                'Remove punctuation',
                'BoW, TF-IDF models',
                'Transformer models (keep structure)',
              ...
              [
                ...
                'May hurt context in transformers'
              ],
              [
                'Spelling correction',
                'User-generated content',
                'Clean corpora, legal/medical text',
                'Fixes OOV and noise'
              ],
              [
                'Lemmatization',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Text Preprocessing</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Text Preprocessing")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'ngram-models': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p>An <strong>n-gram language model</strong> predicts the probability of a word given the previous N-1 words. It is a statistical approach to language modeling based on counting co-occurrences in a corpus. N-grams are the foundation of traditional NLP and illustrate core concepts like probability estimation and smoothing that remain relevant today. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, An <strong>n-gram language model</strong> predicts the probability of a word given the previous N-1 words. It is a statistical approach to language modeling based on counting co-occurrences in a corpus. N-grams are the foundation of traditional NLP and illustrate core concepts like probability estimation and smoothing that remain relevant today. Unigram (N=1): predicts each word independently — P(word) Bigram (N=2): predicts a word given the previous word — P(wordₙ | wordₙ₋₁) Trigram (N=3): predicts a word given the previous two words — P(wordₙ | wordₙ₋₂, wordₙ₋₁) Higher-order n-grams capture more context but suffer from data sparsity</p>',
            '<p>You use n-gram language models when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — N-Gram Language Models

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: N-Gram Language Models sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Sentence probability:
  P("The cat sat") = P(The) × P(cat|The) × P(sat|cat)
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'N-Gram Language Models with Python',
            code: `from sklearn.feature_extraction.text import TfidfVectorizer

docs = ["machine learning is great", "deep learning with neural nets", "natural language processing"]
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("N-Gram Language Models — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>N-Gram Language Models</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — N-Gram Language Models")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'text-classification': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Text classification</strong> is the task of automatically assigning one or more predefined categories or labels to a text document. It is one of the most common and practical NLP applications, powering spam detection, sentiment analysis, topic labeling, intent recognition, and content moderation. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Text classification</strong> is the task of automatically assigning one or more predefined categories or labels to a text document. It is one of the most common and practical NLP applications, powering spam detection, sentiment analysis, topic labeling, intent recognition, and content moderation. Binary classification: two classes — spam vs ham, positive vs negative sentiment Multi-class classification: one label from many categories — news topic categories (sports, politics, tech) Multi-label classification: multiple labels per document — a blog post can be both "AI" and "Healthcare" Classifiers use features from text (words, TF-IDF, embeddings) to learn decision boundaries</p>',
            '<p>You use text classification when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Text Classification

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Text Classification sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
vec = TfidfVectorizer()
X = vec.fit_transform(docs)
print("Text Classification — TF-IDF shape:", X.shape)
print("Vocabulary sample:", vec.get_feature_names_out()[:6])`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
            type: 'code'
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Logistic Regression',
                'Linear',
                'Fast, probabilistic output, regularized',
              ...
              [
                ...
                'Overfits on sparse text',
                'When features are dense'
              ],
              [
                'CNN',
                'Neural',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Text Classification</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Text Classification")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
