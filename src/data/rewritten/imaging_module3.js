...
// Regenerate: node scripts/upgrade-modules.js imaging_module3.js
    ...
    'image-restoration': {
      ...
      sections: [
        ...
        },
        {
          ...
          content: [
            '<p><strong>Image restoration</strong> is the process of recovering a clean image from a degraded observation. Unlike enhancement (which improves visual appeal), restoration aims to invert a known or estimated degradation model to recover the original scene. Common degradations include blur, noise, compression artifacts, and missing pixels. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Image restoration</strong> is the process of recovering a clean image from a degraded observation. Unlike enhancement (which improves visual appeal), restoration aims to invert a known or estimated degradation model to recover the original scene. Common degradations include blur, noise, compression artifacts, and missing pixels. Blur: caused by camera shake, out-of-focus optics, or atmospheric turbulence Noise: sensor noise, photon noise, or transmission errors corrupt pixel values Compression artifacts: blockiness and ringing from aggressive JPEG or video encoding Missing data: occlusions, dead pixels, or damaged regions that must be inpainted</p>',
            '<p>You use image restoration when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Image Restoration

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Image Restoration sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
...
Where K = noise-to-signal power ratio`,
        ...
        },
        {
          ...
          example: {
            title: 'Image Restoration with Python',
            code: `import numpy as np
# Image Restoration — synthetic 16-bit slice
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'Inverse Filter',
                'Direct division in frequency domain',
                'Yes',
                'Simple, exact if no noise',
              ...
              [
                ...
                'Yes',
                'Stable; minimizes MSE',
                'Requires PSF and noise stats'
              ],
              [
                'Richardson-Lucy',
                'Iterative ML for Poisson noise',
                'Yes',
                'Good for photon noise',
                'Slow; may diverge'
              ],
              [
                'Non-local Means',
                'Patch-based self-similarity',
                'No',
                'No blur model needed',
              ...
              [
                'Blind Deconvolution',
                'Estimates PSF + image jointly',
                'No',
                'No PSF required',
                'Ill-posed; many local minima'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        },
        {
          ...
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Image Restoration</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
...
plt.title("Bill by day — Image Restoration")
plt.suptitle("")
plt.show()`,
    ...
    'frequency-domain': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Frequency domain processing</strong> transforms an image from the spatial domain (pixels) to a representation where information is organized by frequency. Low frequencies capture smooth intensity variations; high frequencies capture edges, textures, and fine detail. Operations like filtering, compression, and restoration are often simpler and more efficient in the frequency domain. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Frequency domain processing</strong> transforms an image from the spatial domain (pixels) to a representation where information is organized by frequency. Low frequencies capture smooth intensity variations; high frequencies capture edges, textures, and fine detail. Operations like filtering, compression, and restoration are often simpler and more efficient in the frequency domain. Fourier Transform: decomposes an image into sinusoidal basis functions of varying frequency, amplitude, and phase Discrete Cosine Transform (DCT): real-valued variant used in JPEG compression Wavelet Transform: multi-resolution decomposition capturing both frequency and spatial location Frequency domain enables global operations (like blur and sharpening) to be performed as simple pointwise multiplications</p>',
            '<p>You use frequency domain processing when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Frequency Domain Processing

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Frequency Domain Processing sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Shift for visualization:
  F_shifted(u,v) = F((u+M/2)%M, (v+N/2)%N)`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Frequency Domain Processing with Python',
            code: `import numpy as np
# Frequency Domain Processing — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
          ...
          table: {
            ...
            rows: [
              [
                ...
                'Complex spectrum (mag + phase)',
                'Filtering, analysis, convolution speedup'
              ],
              [
                'DCT',
                'Cosine functions (real)',
                'Real coefficients',
                'Compression (JPEG), energy compaction'
              ],
              [
                'Wavelet',
                'Localized wavelets',
                'Scale-space coefficients',
                'Multi-resolution analysis, denoising'
              ],
              [
                'Gabor',
                'Gaussian-modulated sinusoids',
                'Localized frequency response',
                'Texture analysis, feature extraction'
              ],
              [
                'Laplacian Pyramid',
                'Bandpass images at scales',
                'Multi-scale decomposition',
                'Blending, enhancement'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Frequency Domain Processing</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Frequency Domain Processing")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'morphological-ops': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Morphological operations</strong> are a set of image processing techniques that probe an image with a small shape called a <strong>structuring element</strong> (SE). They operate on the geometry of objects rather than pixel intensities, making them ideal for binary image analysis, noise removal, boundary extraction, and shape simplification. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Morphological operations</strong> are a set of image processing techniques that probe an image with a small shape called a <strong>structuring element</strong> (SE). They operate on the geometry of objects rather than pixel intensities, making them ideal for binary image analysis, noise removal, boundary extraction, and shape simplification. Erosion: shrinks foreground objects by removing pixels at boundaries Dilation: expands foreground objects by adding pixels to boundaries Opening: erosion followed by dilation — removes small noise and breaks thin connections Closing: dilation followed by erosion — fills small holes and smooths contours Structuring element: a small binary mask (disk, square, line) that defines the neighborhood shape</p>',
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              [
                'Opening',
                'Smooths from outside',
                'Noise spikes, thin bridges',
                '—',
                'Noise removal before analysis'
              ],
              [
                'Closing',
                'Smooths from inside',
              ...
              [
                'Top-hat',
                'Small feature isolation',
                'Large objects',
                '—',
                'Detecting small bright/dark spots'
              ]
            ]
          }
        },
    ...
    'feature-extraction': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Feature extraction</strong> is the process of identifying and describing meaningful structures in an image that can be used for matching, recognition, or classification. Good features are <strong>repeatable</strong> (detected under viewpoint/lighting changes), <strong>distinctive</strong> (unique enough to avoid mismatches), and <strong>compact</strong> (low-dimensional for efficient comparison). Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Feature extraction</strong> is the process of identifying and describing meaningful structures in an image that can be used for matching, recognition, or classification. Good features are <strong>repeatable</strong> (detected under viewpoint/lighting changes), <strong>distinctive</strong> (unique enough to avoid mismatches), and <strong>compact</strong> (low-dimensional for efficient comparison). Corners: points where intensity changes sharply in multiple directions (Harris, Shi-Tomasi) Blobs: regions with distinctive scale-space extrema (SIFT, SURF, ORB) Edges: boundaries between regions (Canny, Sobel) Descriptors: numerical vectors that capture the local appearance around each feature point</p>',
            '<p>You use feature extraction when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Feature Extraction

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Feature Extraction sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Interpretation:
   ...
   λ₁ >> 0, λ₂ >> 0: corner`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Feature Extraction with Python',
            code: `import numpy as np
# Feature Extraction — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
          ...
          table: {
            ...
            rows: [
              ...
              [
                ...
                'Translation, rotation',
                'Fast',
                'Optical flow (goodFeaturesToTrack)'
              ],
              [
                'SIFT',
                'Blob + descriptor',
                ...
                'Slow',
                'Robust matching, 3D reconstruction'
              ],
              [
                'SURF',
                'Blob + descriptor',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Feature Extraction</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Feature Extraction")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'texture-analysis': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Texture</strong> refers to the spatial repetition and arrangement of pixel intensities in a region. Unlike color or shape, texture describes the <strong>local variability</strong> and <strong>pattern structure</strong> of a surface. Texture analysis extracts quantitative descriptors that distinguish materials, tissues, and terrains based on their visual patterns. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Texture</strong> refers to the spatial repetition and arrangement of pixel intensities in a region. Unlike color or shape, texture describes the <strong>local variability</strong> and <strong>pattern structure</strong> of a surface. Texture analysis extracts quantitative descriptors that distinguish materials, tissues, and terrains based on their visual patterns. Statistical methods: analyze intensity distributions and co-occurrence (GLCM, Haralick features) Structural methods: model textures as repeating primitives (texels) with placement rules Spectral methods: use frequency domain analysis (Fourier, Gabor filters) to capture periodicity Model-based methods: fit parametric models (autoregressive, Markov random fields) to texture</p>',
            '<p>You use texture analysis when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Texture Analysis

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Texture Analysis sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Homogeneity:
  ...
  → High when off-diagonal elements are small`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Texture Analysis with Python',
            code: `import numpy as np
# Texture Analysis — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
          ...
          table: {
            ...
            rows: [
              [
                ...
                'Medium',
                'Material classification, medical tissue'
              ],
              [
                'LBP',
                'Local binary patterns',
                'Fast',
                'Face recognition, real-time texture'
              ],
              [
                'Gabor Filters',
                'Frequency + orientation',
                'Medium',
                'Directional textures, fingerprint analysis'
              ],
              [
                'Wavelet Energy',
                'Multi-scale energy',
                'Medium',
                'Image segmentation, defect detection'
              ],
              [
                'Deep Features',
                'Hierarchical patterns',
                'Slow (inference)',
                'General texture when training data exists'
              ],
              [
                'Fractal Dimension',
                'Self-similarity at scales',
                'Medium',
                'Natural terrain, roughness measurement'
              ]
            ]
          }
        },
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Texture Analysis</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Texture Analysis")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'image-registration': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Image registration</strong> is the process of geometrically aligning two or more images of the same scene so that corresponding points coincide. It is essential for comparing, combining, or analyzing images acquired at different times, from different viewpoints, or by different sensors. Registration solves for a spatial transformation that maps one image (the moving image) onto another (the fixed/reference image). Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Image registration</strong> is the process of geometrically aligning two or more images of the same scene so that corresponding points coincide. It is essential for comparing, combining, or analyzing images acquired at different times, from different viewpoints, or by different sensors. Registration solves for a spatial transformation that maps one image (the moving image) onto another (the fixed/reference image). Rigid: translation and rotation only (6 DOF in 3D; 3 DOF in 2D) Affine: rigid + scaling + shearing (12 DOF in 3D; 6 DOF in 2D) Projective: preserves straight lines but not parallelism (8 DOF in 2D; homography) Non-rigid / deformable: free-form deformation modeled by B-splines or optical flow</p>',
            '<p>You use image registration when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Image Registration

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Image Registration sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Mutual Information (MI):
  ...
  → Best for multi-modal registration (CT-MRI, MRI-PET)`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Image Registration with Python',
            code: `import numpy as np
# Image Registration — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              [
                'Feature-based',
                'Rigid / affine / homography',
                'Geometric error on landmarks',
                'Fast',
                'Remote sensing, panorama stitching'
              ],
              [
                'Intensity-based',
                'Any',
              ...
              [
                ...
                'Very fast',
                'Shift/rotation estimation'
              ],
              [
                'Optical flow',
                'Dense per-pixel motion',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Image Registration</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Image Registration")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
    ...
    'image-quality': {
      ...
      sections: [
        ...
        },
        {
          heading: 'Concept Explanation',
          content: [
            '<p><strong>Image Quality Assessment (IQA)</strong> is the task of quantifying the visual quality of an image. IQA methods predict how humans would judge quality, or measure technical fidelity against a pristine reference. Quality degradations include blur, noise, compression artifacts, color distortion, and geometric errors. Start with intuition: ask what question this concept answers before memorizing formulas.</p>',
            '<p>Technically, <strong>Image Quality Assessment (IQA)</strong> is the task of quantifying the visual quality of an image. IQA methods predict how humans would judge quality, or measure technical fidelity against a pristine reference. Quality degradations include blur, noise, compression artifacts, color distortion, and geometric errors. Full-reference (FR): a perfect reference image is available; quality is deviation from reference (PSNR, SSIM, LPIPS) Reduced-reference (RR): only partial reference information is available (features, statistics) No-reference (NR) / blind: no reference at all; quality predicted from image statistics alone (BRISQUE, NIQE, deep IQA) Perceptual metrics aim to correlate with human opinion scores (MOS — Mean Opinion Score)</p>',
            '<p>You use image quality assessment when you need reproducible, evidence-based decisions rather than gut feeling — A/B tests, clinical trials, forecasting, and model evaluation all depend on it.</p>'
          ],
          note: 'References: Casella & Berger (2002), <em>Statistical Inference</em>; Wasserman (2004), <em>All of Statistics</em>.'
        },
        {
          heading: 'Visual Representation',
          code: `Concept map — Image Quality Assessment

  Raw data  →  Summarize / model  →  Inference  →  Decision
     |              |                    |              |
  sample n      estimate θ̂          CI / p-value    deploy / report

  Key idea: Image Quality Assessment sits in the inference layer — turning noisy samples into actionable ranges.`,
          language: 'text'
        },
        {
          heading: 'Key Formula / Rule',
...
Range: [-1, 1], with 1 = perfect match.`,
        ...
        },
        {
          heading: 'Python Code Example',
          example: {
            title: 'Image Quality Assessment with Python',
            code: `import numpy as np
# Image Quality Assessment — synthetic 16-bit slice
ct = np.random.randint(-1000, 2000, (512, 512), dtype=np.int16)
print("HU range:", ct.min(), "to", ct.max(), "mean:", round(ct.mean(), 1))`,
            output: 'Run in a notebook; verify shapes, p-values, or metrics match expectations.',
            language: 'python',
          ...
          table: {
            ...
            rows: [
              [
                ...
                'Full',
                'Poor',
                'Technical signal fidelity, compression benchmarking'
              ],
              [
                'SSIM',
                'Structural',
                'Full',
                'Good',
                'General-purpose perceptual quality'
              ],
              [
                'MS-SSIM',
                'Multi-scale SSIM',
                'Full',
                'Better',
                'Varying resolution, scale-sensitive tasks'
              ],
              [
                'LPIPS',
                'Deep perceptual',
                'Full',
                'Excellent',
                'GAN evaluation, neural network outputs'
              ],
              [
                'BRISQUE',
                'Statistical (NR)',
                'None',
                'Good',
                'Blind quality assessment in the wild'
              ],
              [
                'NIQE',
                'Statistical (NR)',
                'None',
                'Good',
                'Natural image quality without training'
              ],
              [
                'FID',
                'Deep statistical',
...
Ans: Statistical significance ≠ practical importance — report the CI and effect size; the result may be significant only because n is huge.`
        ...
        {
          heading: 'Try It Yourself',
          text: '<strong>Task:</strong> Load the seaborn <code>tips</code> dataset, compute a group summary statistic relevant to <em>Image Quality Assessment</em>, and visualize the distribution. Interpret one number from the output.',
          example: {
            title: 'Solution (collapsed)',
            code: `import seaborn as sns
import matplotlib.pyplot as plt
...
print("Categories:", tips["day"].unique())
tips.boxplot(column="total_bill", by="day")
plt.title("Bill by day — Image Quality Assessment")
plt.suptitle("")
plt.show()`,
            output: 'You should see summary stats and a boxplot by day; compare medians and spread before choosing a test.',
            language: 'python',
