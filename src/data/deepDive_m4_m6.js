// Deep dives for modules 5, 6, 7 (shifted after REST insertion)
    ...
    timeouts: [
        ...
        table: {
          ...
          rows: [
            [
              ...
              "5–30 seconds",
              "200 ms–2 s",
              "API gateways use 200 ms; internal microservices use 1–2 s"
            ],
            [
              "Database Query Timeout",
              "10–30 seconds",
              "100 ms–1 s",
              "OLTP queries should complete in <100 ms at scale"
            ],
            [
              "Cache Read Timeout",
              "1–5 seconds",
              "5–50 ms",
              "Redis typically responds in <1 ms; 50 ms is a red flag"
            ],
            [
              "Message Queue Timeout",
              "30–300 seconds",
              "5–30 seconds",
              "Kafka consumer timeouts depend on processing complexity"
            ],
            [
              "DNS Resolution Timeout",
              "5 seconds",
              "1–2 seconds",
              "Cloud DNS (Route 53, Cloud DNS) resolves in <50 ms normally"
            ],
            [
              "Connection Establishment",
              "10 seconds",
              "1–3 seconds",
              "TCP handshake + TLS; 1 RTT (~50 ms) ideal"
            ],
            [
              "Circuit Breaker Open Duration",
              "30–60 seconds",
              "5–15 seconds",
              "Exponential backoff after failure threshold"
            ],
            [
              "Health Check Timeout",
              "5 seconds",
              "1–2 seconds",
              "Kubernetes liveness probes default to 1 s"
            ],
            [
              "Bulkhead Thread Pool Timeout",
              "10–60 seconds",
              "1–5 seconds",
              "Hystrix default: 1 s execution timeout"
            ],
            [
              "Total End-to-End Timeout",
              "30–120 seconds",
              "2–10 seconds",
              "User-facing: 2 s; async batch: 10 s+"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              "1× (Baseline)",
              "< 200 ms",
              "All requests succeed within timeout",
              "Healthy system, no timeout pressure"
            ],
            [
              "2× Load",
              "200 ms–1 s",
              "5–10% of requests hit soft timeout warnings",
              "Monitor p95/p99 latencies closely"
            ],
            [
              "5× Load",
              "1–5 s",
              "20–40% requests timeout and retry",
              "Circuit breakers start tripping; queues build up"
            ],
            [
              "10× Load",
              "> 5 s or ∞",
              "50–80% requests fail with timeout",
              "Cascading failure risk; bulkheads prevent total collapse"
            ],
            [
              "Burst Traffic",
              "Spikes to 10 s+",
              "Timeout storms trigger retry amplification",
              "Exponential backoff + jitter essential"
            ]
          ]
        }
      },
      ...
      },
      {
        ...
        table: {
          ...
          rows: [
            [
              "User-facing API (sync)",
              "200 ms–1 s",
              "Users abandon after 3 s; 200 ms keeps p99 happy",
              "API Gateway / NGINX"
            ],
            [
              "Internal service call",
              "1–5 s",
              "Network + processing overhead; retries possible",
              "Resilience4j / Hystrix"
            ],
            [
              "Database query (OLTP)",
              "100 ms–1 s",
              "Slow queries indicate missing index or lock contention",
              "PostgreSQL statement_timeout"
            ],
            [
              "Cache read",
              "5–50 ms",
              "Cache miss should fallback quickly; slow cache = worse than no cache",
              "Redis timeout config"
            ],
            [
              "Async batch job",
              "30 s–5 min",
              "Large data processing needs headroom; monitor progress",
              "Celery / AWS Batch"
            ],
            [
              "Cross-region call",
              "RTT + 2× processing time",
              "US-East to US-West RTT ~70 ms; add buffer",
              "Latency-aware routing"
            ],
            [
              "Mobile app API",
              "5–10 s",
              "Mobile networks are flaky; allow longer but retry with backoff",
              "OkHttp / Alamofire timeouts"
            ]
          ]
        }
      }
        ...
        table: {
          ...
          rows: [
            [
              ...
              "3–5",
              "1–3",
              "Fewer retries at scale to reduce amplification"
            ],
            [
              "Base Retry Delay",
              "100 ms–1 s",
              "10–100 ms",
              "Exponential backoff starts small at hyperscale"
            ],
            [
              "Max Retry Delay",
              "10–60 s",
              "1–5 s",
              "Cap backoff to prevent indefinite delay"
            ],
            [
              "Retry Jitter Range",
              "±10–25%",
              "±50–100%",
              "Full jitter prevents synchronization at scale"
            ],
            [
              "Retry Budget (per minute)",
              "10% of requests",
              "1–5% of requests",
              "Spotify uses 1% retry budget to prevent overload"
            ],
            [
              "Idempotency Key TTL",
              "1–24 hours",
              "5–60 minutes",
              "Long enough for retries, short enough for storage"
            ],
            [
              "Retry Storm Threshold",
              "100 req/s",
              "1,000+ req/s",
              "Rate limit retries separately from normal traffic"
            ],
            [
              "Circuit Breaker After Retries",
              "5 failures in 10 s",
              "50 failures in 10 s",
              "Scale threshold with request volume"
            ],
            [
              "Total Retry Timeout",
              "30 s–2 min",
              "5–30 s",
              "User-facing: fast fail; async: longer tolerance"
            ],
            [
              "Per-Attempt Timeout",
              "1–5 s",
              "100 ms–1 s",
              "Each attempt must be shorter than total budget"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "Normal + 0–100 ms",
              "0–1% retry rate",
              "Healthy; retries are invisible to users"
            ],
            [
              "2× Load",
              "Normal + 100 ms–1 s",
              "5–10% retry rate",
              "First signs of strain; jitter prevents sync"
            ],
            [
              "5× Load",
              "1–5 s (with retries)",
              "20–30% retry rate",
              "Retry amplification begins; consider circuit breaker"
            ],
            [
              "10× Load",
              "> 10 s or failed",
              "40–60% retry rate",
              "Retry storm risk; bulkhead isolation critical"
            ],
            [
              "Retry Storm",
              "System-wide degradation",
              "100% retry rate on failing service",
              "Without backoff, retries DDoS the service"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "0–1 retry",
              "None or fixed 1 s",
              "Must use idempotency key; avoid duplicate writes"
            ],
            [
              "Payment processing",
              "0 retries",
              "N/A",
              "Fail fast; manual reconciliation instead"
            ],
            [
              "Real-time chat message",
              "1 retry, 100 ms",
              "Fixed",
              "User is waiting; fast fail better than delay"
            ],
            [
              "Batch data import",
              "5–10 retries",
              "Exponential up to 5 min",
              "Background job; can afford longer waits"
            ],
            [
              "Mobile API (unreliable network)",
              "3 retries, 5 s timeout",
              "Exponential + full jitter",
              "Network flakiness is normal; be generous"
            ],
            [
              "Internal service mesh (Istio)",
              "2 retries",
              "Automatic (Envoy default)",
              "Envoy handles retries transparently"
            ]
          ]
        }
      }
        ...
        table: {
          ...
          rows: [
            [
              ...
              "16–64 bytes",
              "32 bytes (UUID)",
              "Stripe uses 64-char string keys"
            ],
            [
              "Key Storage TTL",
              "1–24 hours",
            ...
            [
              ...
              "0.1–1%",
              "5–10% during incidents",
              "Spikes during network partitions"
            ],
            [
              "Idempotency Store QPS",
              "100–1,000",
              "10,000–100,000",
              "Redis handles 100K+ ops/sec per node"
            ],
            [
            ...
            [
              ...
              "100 bytes–1 KB",
              "50–200 bytes",
              "Key + request hash + response pointer"
            ],
            [
              "Lock Duration (pessimistic)",
              "1–10 s",
              "100 ms–1 s",
              "Short locks prevent deadlock"
            ],
            [
              "Optimistic Retry Window",
              "30 s–5 min",
              "5–30 s",
            ...
            [
              ...
              "95–99%",
              "99.9%",
              "Most requests are first-time; duplicates are rare"
            ],
            [
              "End-to-End Idempotency Latency",
              "+1–5 ms",
              "+0.1–1 ms",
              "Redis lookup overhead at scale"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              "1× (Baseline)",
              "Normal + 1–5 ms",
              "0.1% duplicates detected",
              "Minimal overhead; cache hits are fast"
            ],
            [
              "2× Load",
              "Normal + 2–10 ms",
              "0.5% duplicates",
              "Slight cache pressure; Redis scales linearly"
            ],
            [
              "5× Load",
              "Normal + 5–20 ms",
              "2–5% duplicates",
              "Network partition recovery phase; duplicates spike"
            ],
            [
              "10× Load",
              "Normal + 10–50 ms",
              "10–20% duplicates",
              "Retry storms create duplicate bursts; TTL critical"
            ],
            [
              "Duplicate Flood",
              "50–200 ms",
              "50–100% duplicates",
              "Client retry bug; rate limit idempotency checks"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "50–60%",
              "70–80%",
              "Higher at scale due to noise; Netflix uses 50%"
            ],
            [
              "Failure Threshold (count)",
              "5 in 10 s",
              "50–100 in 10 s",
              "Count-based for low-volume services"
            ],
            [
              "Slow Call Threshold (%)",
              "80–100%",
              "60–80%",
              "Percentage of calls > slow call duration"
            ],
            [
              "Slow Call Duration",
              "1–5 s",
              "100 ms–1 s",
              "Hystrix default: 1 s; hyperscale: 100 ms"
            ],
            [
              "Open Duration (fixed)",
              "30–60 s",
              "5–15 s",
              "How long breaker stays open before half-open"
            ],
            [
              "Open Duration (exponential)",
              "10 s → 20 s → 40 s",
              "1 s → 2 s → 4 s",
              "Backoff for persistent failures"
            ],
            [
              "Half-Open Permitted Calls",
              "1–3",
              "5–10",
              "Test calls to check recovery"
            ],
            [
              "Breaker Window Size",
              "10–60 s",
            ...
            [
              "Minimum Calls to Trip",
              "5–10",
              "20–100",
              "Prevent tripping on first few failures"
            ],
            [
              "Fallback Timeout",
              "10–100 ms",
              "1–10 ms",
              "Fallback must be faster than normal path"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "No failure threshold reached"
            ],
            [
              "2× (Degraded)",
              "Normal + fallback latency",
              "CLOSED → some failures",
              "p99 latency rises; monitor failure rate"
            ],
            [
              "5× (Overloaded)",
              "Fallback (< 10 ms)",
              "OPEN — requests fail fast",
              "Service protected; fallback serves stale data"
            ],
            [
              "10× (Downstream Down)",
              "Fallback only",
              "OPEN — 100% fallback",
              "Downstream completely bypassed; monitor recovery"
            ],
            [
              "Recovery Phase",
              "Gradual improvement",
              "HALF-OPEN → CLOSED",
              "Test calls succeed; breaker re-closes"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "10 s",
              "10 s",
              "Database fallback (slower but safe)"
            ],
            [
              ...
              "10 s",
              "30 s",
              "Envoy automatic retry to alternate"
            ]
          ]
        }
      }
    ...
    bulkhead: [
      {
        ...
        table: {
          ...
          rows: [
            [
              "Thread Pool Size (per service)",
              "10–20",
              "5–10",
              "Smaller pools at scale; more instances instead"
            ],
            [
              "Thread Pool Queue Size",
              "5–10",
              "0–5",
              "Queue size 0 = fail fast; Netflix prefers 0"
            ],
            [
            ...
            [
              ...
              "1–2 cores",
              "0.5–1 core",
              "Kubernetes: 500m–1000m CPU request"
            ],
            [
              "Container Memory Limit",
              "1–4 GB",
              "512 MB–1 GB",
              "Smaller containers = faster scaling"
            ],
            [
              "Max Connections per Pod",
              "50–100",
              "10–20",
              "NGINX worker: 768 default; tune down"
            ],
            [
              "DB Connection Pool per Service",
              "10–20",
              "5–10",
              "Total pool size < DB max_connections"
            ],
            [
              "Bulkhead Timeout",
              "1–5 s",
        ...
        table: {
          ...
          rows: [
            [
              "1× (Baseline)",
              "Normal",
              "All requests within pool capacity",
              "Healthy; pool utilization < 50%"
            ],
            [
              "2× Load",
              "Normal + queue wait",
              "Some requests queue briefly",
              "Queue absorbs spike; no failures yet"
            ],
            [
              "5× Load",
              "Timeout or rejection",
              "Pool saturated; new requests rejected",
              "Fail fast prevents resource exhaustion"
            ],
            [
              "10× Load",
              "Immediate rejection",
              "100% rejection after pool + queue full",
              "Other services unaffected; bulkhead working"
            ],
            [
              "Cascading Failure Without Bulkhead",
              "System-wide outage",
              "All services starved for threads",
              "Without bulkhead: one slow service kills all"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "5–10",
              "0",
              "Fail fast; never queue payments"
            ],
            [
              "User analytics",
              "Semaphore",
              "50–100",
              "Bounded",
              "Lightweight; tolerate brief delays"
            ],
            [
              ...
              "10",
              "0",
              "External latency unpredictable; isolate"
            ],
            [
              "Database reads",
              "Connection pool",
              "10–20",
              "5",
              "Queue brief spikes; fail on sustained load"
            ],
            [
              "Database writes",
              "Connection pool",
              "5–10",
              "0",
              "Writes are expensive; fail fast"
            ],
            [
              ...
              "5",
              "0",
              "Large memory footprint; strict limits"
            ],
            [
              ...
              "10–100 pods",
              "N/A",
              "Prevent namespace from consuming cluster"
            ]
          ]
        }
      }
        ...
        table: {
          ...
          rows: [
            [
              ...
              "100–1,000 req/min",
              "10–100 req/s",
              "GitHub: 5,000 req/hour; Stripe: 100 req/s"
            ],
            [
              "Per-IP Rate Limit",
              "60–600 req/min",
              "10–30 req/s",
              "Cloudflare: free tier 1,200 req/5 min"
            ],
            [
              "Per-API Key Rate Limit",
              "1,000–10,000 req/min",
              "100–1,000 req/s",
              "Enterprise keys get higher limits"
            ],
            [
              "Burst Allowance",
              "10–50% over limit",
              "0–10%",
              "Token bucket allows brief bursts"
            ],
            [
              "Rate Limit Window",
              "1–60 seconds",
              "1 second",
              "Sliding window for accuracy; fixed for performance"
            ],
            [
              "Rate Limit Key Space",
              "1M–10M keys",
              "100M–1B keys",
              "Redis Cluster handles billions of keys"
            ],
            [
              "Penalty Duration (exceeded)",
              "1–60 minutes",
              "1–5 minutes",
              "Progressive penalty: 1 min → 5 min → 1 hour"
            ],
            [
              "Rate Limit Check Latency",
              "1–5 ms",
              "0.1–1 ms",
              "Redis INCR + EXPIRE: <1 ms"
            ],
            [
              "Distributed Rate Limit Sync",
              "1–5 s",
              "100 ms–1 s",
              "Gossip protocol or Redis pub/sub"
            ],
            [
              "Global Rate Limit",
              "10K–100K req/s",
              "1M–10M req/s",
              "CDN edge rate limiting (Cloudflare, Fastly)"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "Normal + 1 ms",
              "100% accepted",
              "Rate limit check is negligible overhead"
            ],
            [
              "2× Limit",
              "Normal + 1 ms",
              "50% accepted, 50% rejected (429)",
              "Fair sharing; legitimate users unaffected"
            ],
            [
              "5× Limit",
              "Normal + 1 ms",
              "20% accepted, 80% rejected",
              "Aggressive rejection; consider scaling or higher limits"
            ],
            [
              "10× Limit",
              "1 ms (rejection)",
              "10% accepted, 90% rejected",
              "DDoS or misconfigured client; alert security"
            ],
            [
              "Attack Traffic",
              "1 ms (rejection)",
              "99%+ rejected at edge",
              "Cloudflare blocks 70M req/s DDoS attacks"
            ]
          ]
        }
      },
    ...
    "horizontal-vertical": [
      {
        ...
        table: {
          ...
          rows: [
            [
              "Max Single Instance",
              "128–768 vCPUs, 24 TB RAM",
              "Unlimited (theoretically)",
              "AWS u-24tb1.metal: 448 vCPUs, 24 TB"
            ],
            [
              "Cost per vCPU/hour",
              "$0.01–$0.05",
              "$0.005–$0.02",
              "Horizontal: spot instances 70% cheaper"
            ],
            [
              "Scale-up Time",
              "5–15 minutes (reboot)",
              "1–5 minutes (new instance)",
              "Vertical: downtime required usually"
            ],
            [
              "Scale-down Time",
              "5–15 minutes",
              "30–60 seconds",
              "Horizontal: auto-terminate unused"
            ],
            [
              "Max Throughput (single)",
              "100K–500K req/s",
              "1M–100M+ req/s",
              "Google Cloud Load Balancer: 1M+ req/s"
            ],
            [
              "Data Locality",
              "Single node = fast",
              "Network latency: 0.5–2 ms",
              "Cross-AZ: 1–2 ms; cross-region: 50–100 ms"
            ],
            [
              "Fault Tolerance",
              "Single point of failure",
              "N-1 redundancy",
              "Horizontal: 3 instances minimum for HA"
            ],
            [
              "Storage Scaling",
              "Add disks to single node",
              "Distributed storage (S3, GFS)",
              "S3: unlimited storage, 11 nines durability"
            ],
            [
              "Network Bandwidth",
              "25–100 Gbps per instance",
              "Aggregate: 1–10 Tbps",
              "AWS EC2: 100 Gbps max per instance"
            ],
            [
              "Operational Complexity",
              "Low",
              "High (orchestration, state)",
              "Kubernetes adds ~20% overhead"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "3 small instances",
              "Both handle load comfortably"
            ],
            [
              "2× Load",
              "Upgrade to larger instance",
              "Add 3 more instances",
              "Vertical: brief downtime; Horizontal: seamless"
            ],
            [
              "5× Load",
              "Max instance size reached",
              "Scale to 15 instances",
              "Vertical hits ceiling; Horizontal keeps growing"
            ],
            [
              "10× Load",
              "Cannot scale further",
              "30+ instances across regions",
              "Vertical: bottleneck; Horizontal: elastic"
            ],
            [
              "100× Load",
              "Complete rewrite needed",
              "300+ instances + auto-scaling",
              "Horizontal with auto-scaling wins"
            ],
            [
              "Burst (10× spike)",
              "Over-provisioned waste",
              "Scale out in 2–5 minutes",
              "Horizontal: pay for burst only"
            ]
          ]
        }
      },
      ...
      },
      {
        ...
        table: {
          ...
          rows: [
            [
              "Early startup (< 1K users)",
              "Vertical",
              "c5.large → c5.2xlarge",
              "Simple, cost-effective, no ops overhead",
              "MVP on single server"
            ],
            [
              "Stateless web API",
              "Horizontal",
              "c5.large × 3–10",
              "Perfect horizontal scaling; add instances",
              "REST API microservices"
            ],
            [
              "Relational database",
              "Vertical + read replicas",
              "db.r5.2xlarge + 3 replicas",
              "ACID needs single writer; scale reads",
            ...
            [
              ...
              "Vertical first",
              "m5.8xlarge (32 vCPU)",
              "Refactor cost high; scale up temporarily",
            ...
            [
              "GPU ML training",
              "Vertical (single node) or Horizontal (distributed)",
              "p4d.24xlarge",
              "Single node: 8 A100 GPUs; Multi-node: Horovod",
              "GPT training"
            ]
          ]
        }
        ...
        table: {
          ...
          rows: [
            [
              ...
              "1–10 Gbps",
              "100+ Gbps",
              "AWS NLB: 100 Gbps per AZ pair"
            ],
            [
              "Layer 7 Throughput",
              "100K–1M req/s",
              "10M+ req/s",
              "Google Cloud LB: 1M+ req/s per instance"
            ],
            [
              "Connection Count",
              "10K–100K",
            ...
            [
              ...
              "< 1 ms",
              "< 0.1 ms",
              "Kernel bypass (DPDK): sub-microsecond"
            ],
            [
              "Latency (L7)",
              "1–5 ms",
              "0.5–2 ms",
              "HTTP parsing + routing adds overhead"
            ],
            [
              "Health Check Interval",
              "5–30 s",
              "1–5 s",
              "Faster detection = more control plane load"
            ],
            [
              "Failover Time",
              "1–30 s",
              "1–5 s",
              "DNS TTL affects failover; anycast: <1 s"
            ],
            [
              "Backend Pool Size",
              "2–10",
        ...
        table: {
          ...
          rows: [
            ...
            [
              "2× Load",
              "2–10 ms",
              "Some backends at 70% capacity",
              "Weighted routing kicks in; consider scale-out"
            ],
            [
              "5× Load",
              "5–20 ms",
              "Backends at 90%+; queueing begins",
              "Add backends or enable auto-scaling"
            ],
            [
              "10× Load",
              "20–100 ms",
              "LB itself becomes bottleneck",
              "Need multiple LBs or anycast distribution"
            ],
            [
              "100× Load",
              "LB upgrade needed",
              "DDoS mitigation mode",
              "Cloudflare absorbs 70M req/s attacks"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "Cloud L4 (NLB)",
              "5-tuple hash",
              "Millions of connections, low latency",
              "Gaming backend"
            ],
            [
              "HTTP routing",
            ...
            [
              ...
              "Proxy (ProxySQL)",
              "Least-connections",
              "Query caching, read/write split",
              "E-commerce DB"
            ],
            [
              "WebSocket/Real-time",
            ...
            [
              ...
              "Global Accelerator",
              "Performance",
              "Optimize for mobile network paths",
              "Uber, Lyft"
            ]
          ]
        }
    ...
    "auto-scaling": [
      {
        ...
        table: {
          ...
          rows: [
            [
              "Scale-out Trigger (CPU)",
              "70–80%",
              "50–60%",
              "Lower threshold = faster response, more cost"
            ],
            [
              "Scale-in Trigger (CPU)",
              "30–40%",
              "20–30%",
              "Prevent flapping; wide hysteresis band"
            ],
            [
              "Scale-out Cooldown",
              "2–5 minutes",
              "30–60 seconds",
              "Faster with predictive scaling"
            ],
            [
              "Scale-in Cooldown",
              "5–15 minutes",
              "10–30 minutes",
              "Longer to prevent premature termination"
            ],
            [
              "Instance Launch Time",
              "2–5 minutes",
              "30–60 seconds",
              "Pre-warmed AMIs; container startup faster"
            ],
            [
              "Target Tracking Response",
              "1–2 minutes",
              "30–60 seconds",
              "AWS Target Tracking: 1-minute CloudWatch"
            ],
            [
              "Predictive Scaling Horizon",
              "N/A",
              "1–24 hours",
              "AWS Predictive: forecast based on history"
            ],
            [
              "Min Instances",
              "2–3",
              "10–50",
              "Higher minimum for baseline + HA"
            ],
            [
              "Max Instances",
              "10–50",
              "1,000–10,000",
              "AWS Auto Scaling: 1,000+ per group"
            ],
            [
              "Cost Savings (vs static)",
              "20–40%",
              "50–70%",
              "Spot instances + auto-scaling = maximum savings"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              "1× (Steady)",
              "Normal",
              "Current instance count stable",
              "No scaling action; baseline cost"
            ],
            [
              "2× Load (gradual)",
              "Slight increase then normal",
              "Scale out in 2–5 minutes",
              "Target tracking handles smoothly"
            ],
            [
              "5× Load (gradual)",
              "1–2 min spike, then normal",
              "Scale out to 3× instances",
              "Some queueing during launch window"
            ],
            [
              "10× Load (sudden)",
              "5–30 s spike",
              "Predictive or scheduled scaling needed",
              "Reactive scaling too slow; pre-warm required"
            ],
            [
              "10× Load (predicted)",
              "Normal",
              "Instances ready before traffic hits",
              "Scheduled scaling for known events"
            ],
            [
              "Load Drop",
              "Normal",
              "Scale in after 10–15 min cooldown",
              "Cost optimization; watch for flapping"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              "Max Connections",
              "100–1,000",
              "100–1,000 per replica",
              "Aggregate: 10K+",
              "PostgreSQL: 100 default; tune to 500"
            ],
            [
              ...
              "Same as primary",
              "10K–100K TPS",
              "MySQL: 5K–10K TPS on modern hardware"
            ],
            [
              "Max Read Throughput",
              "10K–50K QPS",
            ...
            [
              ...
              "Same as primary",
              "100 TB+",
              "PostgreSQL: practical limit ~4 TB"
            ],
            [
              "Replication Lag",
              "N/A",
              "1 ms–1 s",
              "N/A (shards independent)",
              "Async replication: 1 ms–1 s typical"
            ],
            [
              "Failover Time",
              "5–15 minutes",
              "1–30 seconds",
              "1–5 minutes",
              "RDS Multi-AZ: 60–120 s failover"
            ],
            [
              "Scale-out Time",
              "5–15 minutes",
        ...
        table: {
          ...
          rows: [
            [
              ...
              "Normal",
              "Normal",
              "All approaches handle load"
            ],
            [
              "2× Read Load",
              "CPU 80%, latency ↑",
              "Distribute to 2 replicas",
              "Normal",
            ...
            [
              "5× Read Load",
              "Unusable",
              "5 replicas, 20% utilization",
              "Normal",
              "Sharding overkill for reads alone"
            ],
            [
              "2× Write Load",
              "CPU 80%, disk IO ↑",
            ...
            [
              ...
              "Primary dead",
              "5 shards, normal",
              "Only sharding handles write scale"
            ],
            [
              "10× Mixed Load",
              "Dead",
              "Primary dead",
              "10 shards, 50% utilization",
              "Sharding + replicas = ultimate scale"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "5–10",
              "10–50",
              "Amazon: 2-pizza team per service"
            ],
            [
              "Deployments per Day",
              "1–10",
              "100–1,000",
              "10,000+",
              "Netflix: 1,000+ production deploys/day"
            ],
            [
              ...
              "10–50",
              "100–1,000",
              "Kubernetes: 3 pods minimum per service"
            ],
            [
              "Inter-service Calls/request",
              "1–3",
              "5–10",
              "10–50",
              "Each user request fans out to many services"
            ],
            [
            ...
            [
              ...
              "5–20 ms",
              "10–50 ms",
              "Adds overhead per hop; caching critical"
            ],
            [
              ...
              "1–3 ms",
              "0.5–2 ms",
              "Istio/Envoy: 1–3 ms per hop at scale"
            ],
            [
              ...
              "Dedicated",
              "Database per service ideal but costly"
            ],
            [
              ...
              "Medium",
              "High (orchestrated)",
              "API contracts, versioning critical"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "60 ms",
              "300 ms",
              "Queueing at each service compounds"
            ],
            [
              "5× Load",
              "20 ms",
            ...
            [
              ...
              "500 ms",
              "5 s+",
              "Circuit breakers open; fallbacks activate"
            ],
            [
              "Cascading Failure",
              "Timeout",
              "Partial outage",
              "System-wide degradation",
              "One slow service affects all callers"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "Single codebase",
              "Multiple codebases",
              "< 10 developers = monolith; > 30 = microservices",
            ...
            [
              ...
              "Shared database",
              "Database per service",
              "Strong consistency needs = shared; independent scaling = per service",
              "Banking vs Social"
            ],
            [
              "Communication",
              "In-process calls",
              "HTTP/gRPC/messaging",
              "Latency sensitive = in-process; resilient = async messaging",
              "Real-time vs E-commerce"
            ],
            [
              "Deployment",
              "Weekly/monthly",
              "Multiple per day",
              "Risk tolerance; feature velocity",
              "Enterprise vs Netflix"
            ],
            [
              "Team Structure",
              "1 large team",
              "2-pizza teams",
              `Conway's Law: architecture follows org`,
            ...
            [
              ...
              "Simple logging",
              "Distributed tracing",
              "> 10 services = tracing mandatory",
              "Jaeger, Zipkin"
            ],
            [
              "Testing",
              "Integration tests",
              "Contract tests + chaos",
        ...
        table: {
          ...
          rows: [
            [
              ...
              "5–30 seconds",
              "1–5 seconds",
              "Container: 1–5 s; VM: 30–60 s"
            ],
            [
              "Instance Memory",
              "256 MB–2 GB",
              "128 MB–512 MB",
              "Smaller = faster scale, more density"
            ],
            [
              "Instance CPU",
              "0.5–2 vCPUs",
              "0.25–1 vCPU",
              "AWS Fargate: 0.25 vCPU minimum"
            ],
            [
              "Max Instances per Service",
              "10–50",
              "1,000–10,000",
              "Kubernetes: 100 pods per namespace typical"
            ],
            [
              "Requests per Instance",
              "100–1,000 req/s",
              "1,000–10,000 req/s",
              "Go: 10K+ req/s; Node.js: 1–5K req/s"
            ],
            [
              "Session Data Size",
              "0 (stateless)",
              "0 (stateless)",
              "All state externalized to Redis/DB"
            ],
            [
              "Scale-out Time",
              "1–5 minutes",
              "10–30 seconds",
              "Kubernetes HPA: 30 s reaction time"
            ],
            [
              "Scale-in Time",
              "5–10 minutes",
              "1–5 minutes",
              "Longer to prevent flapping"
            ],
            [
              "Rolling Update Time",
              "5–15 minutes",
              "1–5 minutes",
              "Kubernetes: maxUnavailable + maxSurge"
            ],
            [
              "Cost per 1K req",
              "$0.001–$0.01",
        ...
        table: {
          ...
          rows: [
            [
              "1× (Baseline)",
              "Normal",
              "3 instances handle load",
              "Even distribution; no session affinity needed"
            ],
            [
              "2× Load",
              "Normal",
              "Scale to 6 instances in 2 min",
              "Seamless; no state migration needed"
            ],
            [
              "5× Load",
              "Normal",
              "15 instances; all healthy",
              "Perfect horizontal scaling; each instance identical"
            ],
            [
              "10× Load",
              "Normal",
              "30 instances; auto-scaling",
              "No bottleneck from shared state"
            ],
            [
              "100× Load",
              "Slight increase",
              "300 instances; cost spike",
              "Stateless scales linearly; watch costs"
            ],
            [
              "Instance Failure",
              "No impact",
              "Traffic redistributed",
              "Other instances unaffected; user sees no difference"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "3–10",
              "10–50",
              "More steps = more complexity, longer execution"
            ],
            [
              "Saga Execution Time",
              "100 ms–1 s",
              "1–30 s",
              "Choreography: faster; Orchestration: more reliable"
            ],
            [
              "Compensating Action Time",
              "10 ms–1 s",
              "100 ms–5 s",
              "Must complete successfully; no retries on comp"
            ],
            [
              "Saga Timeout",
              "5–30 s",
              "30 s–5 min",
              "Longer for external service calls"
            ],
            [
              "Concurrent Sagas",
              "100–1,000",
              "10,000–100,000",
              "Temporal: 100K+ concurrent workflows"
            ],
            [
              "Saga Failure Rate",
              "0.1–1%",
              "1–5%",
              "Higher with more steps and external services"
            ],
            [
              "Compensation Success Rate",
              "99.9%",
              "99.5%",
              "Compensation must be designed to not fail"
            ],
            [
              "Event Store Retention",
              "7–30 days",
              "1–7 days",
              "Saga events for debugging/auditing"
            ],
            [
              "Saga Orchestrator Latency",
              "1–5 ms",
              "< 1 ms",
              "In-memory state machine; minimal overhead"
            ],
            [
              "Message Queue for Saga",
              "1,000–10,000 msg/s",
              "100,000+ msg/s",
              "Kafka: 1M+ msg/s per cluster"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              "1× (Baseline)",
              "100 ms–1 s",
              "99% sagas complete successfully",
              "Normal operation; occasional compensation"
            ],
            [
              "2× Load",
              "100 ms–2 s",
              "98% success; 2% compensation",
              "Slight increase in partial failures"
            ],
            [
              "5× Load",
              "1–5 s",
              "95% success; 5% compensation",
              "External services slow down; more timeouts"
            ],
            [
              "10× Load",
              "5–30 s",
              "90% success; 10% compensation",
              "Compensations cascade; monitor carefully"
            ],
            [
              "Saga Storm",
              "30 s–timeout",
              "50% success; 50% compensation",
              "Circuit breaker on saga steps needed"
            ],
            [
              "Compensation Failure",
              "Manual intervention",
              "Data inconsistency risk",
              "Escalate to human; audit log critical"
            ]
          ]
        }
      },
    ...
    "two-phase-commit": [
      {
        ...
        table: {
          ...
          rows: [
            [
              "2PC Latency (no failure)",
              "2–3 network RTTs",
              "4–6 network RTTs",
              "Prepare + Commit phases"
            ],
            [
              "2PC Latency (with failure)",
              "5–10 s",
              "10–30 s",
            ...
            [
              ...
              "2–5",
              "5–10",
              "More participants = higher failure probability"
            ],
            [
            ...
            [
              ...
              "0.01–0.1%",
              "0.1–1%",
              "Coordinator failure during commit = blocking"
            ],
            [
              "Throughput (2PC)",
              "100–1,000 TPS",
              "10–100 TPS",
              "Per distributed transaction; much slower than local"
            ],
            [
              "Throughput (local TX)",
              "1,000–10,000 TPS",
            ...
            [
              ...
              "4 (2 per participant)",
              "2N (N participants)",
              "Prepare log + commit log"
            ],
            [
              "Network Messages per 2PC",
              "4N",
              "4N + coordination",
            ...
            [
              ...
              "1–5 s",
              "5–10 s",
              "How long to wait for prepare votes"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              "2× Load",
              "20–100 ms",
              "95% commit, 5% timeout retry",
              "Some prepare phases timeout"
            ],
            [
              "5× Load",
              "100 ms–1 s",
              "80% commit, 20% abort/timeout",
              "Coordinator saturated; consider sharding"
            ],
            [
              "10× Load",
              "1–5 s",
              "50% commit, 50% issues",
              "2PC coordinator is single bottleneck"
            ],
            [
              "Coordinator Failure",
              "Blocking",
              "All in-flight transactions block",
              "Until coordinator recovers; minutes possible"
            ],
            [
              "Participant Failure",
              "Transaction abort",
              "Automatic rollback",
              "Safer than blocking; but work lost"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "1",
              "2–3",
              "W = N for all-node writes"
            ],
            [
              "Read Latency (local)",
              "1–2 ms",
              "2–5 ms",
              "R=1: single node; R=3: wait for majority"
            ],
            [
              "Write Latency (local)",
              "1–2 ms",
              "5–10 ms",
              "W=1: fast; W=3: wait for replication"
            ],
            [
              "Read Latency (cross-region)",
              "50–100 ms",
              "100–200 ms",
              "Quorum across regions = high latency"
            ],
            [
              "Write Latency (cross-region)",
              "50–100 ms",
              "100–500 ms",
              "DynamoDB global tables: 1 s replication"
            ],
            [
              "Availability (1 node down)",
              "100%",
              "100% (if R+W≤N+1)",
              "Quorum tolerates (N-1)/2 failures"
            ],
            [
              "Availability (2 nodes down, N=3)",
              "Degraded",
              "Unavailable",
              "Strong quorum needs majority"
            ],
            [
              "Stale Read Probability",
              "10–50%",
              "0%",
              "R=1: high stale read chance"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "3–8 ms",
              "8–15 ms",
              "Strong quorum latency increases"
            ],
            [
              "5× Load",
              "2–5 ms",
              "10–30 ms",
              "30–100 ms",
              "Quorum waits for slowest replica"
            ],
            [
              "10× Load",
              "5–10 ms",
              "50–200 ms",
              "Timeout",
              "Strong quorum may fail under extreme load"
            ],
            [
              "Node Failure (1 down)",
              "Normal",
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "Medium",
              "High",
              "TCC requires 3 operations per service"
            ],
            [
              ...
              "Optional",
              "Yes",
              "Saga: choreography needs no coordinator"
            ],
            [
              "Blocking Risk",
              "High",
            ...
            [
              ...
              "Eventual",
              "Eventual",
              "Saga: temporary inconsistency during execution"
            ],
            [
              ...
              "Yes",
              "Yes (Cancel)",
              "Saga and TCC need compensating logic"
            ],
            [
              "Industry Usage",
              "Declining",
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "1 s–1 min",
              "1 min–1 hour",
              "Time until all replicas converge"
            ],
            [
            ...
            [
              "Monotonic Read Latency",
              "1–5 ms",
              "1–10 ms",
              "Guarantee: reads see increasing writes"
            ],
            [
              "Stale Read Probability",
              "1–10%",
              "10–50%",
              "Depends on write rate and replication lag"
            ],
            [
              "Conflict Resolution Time",
              "1–10 ms",
              "1–100 ms",
              "Last-write-wins: fast; CRDT: depends"
            ],
            [
              "Anti-Entropy Repair Rate",
              "1–10 MB/s",
              "100 MB/s–1 GB/s",
              "Cassandra: background repair"
            ],
            [
              "Gossip Protocol Interval",
              "1 s",
              "100 ms–1 s",
              "Cassandra: 1 s gossip interval"
            ],
            [
              "Vector Clock Size",
              "100 bytes–1 KB",
              "1–10 KB",
              "Per-replica entry; grows with replicas"
            ],
            [
              "Eventual Consistency Throughput",
              "10K–100K ops/s",
              "1M–10M ops/s",
              "Cassandra: 1M+ writes/s per node"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "1–2 ms",
              "1 s",
              "Fast reads/writes; quick convergence"
            ],
            [
              "2× Load",
              "1–2 ms",
              "1–2 ms",
              "1–5 s",
              "Replication lag increases slightly"
            ],
            [
              "5× Load",
              "2–5 ms",
              "1–2 ms",
              "5–30 s",
              "Write replication backpressure"
            ],
            [
              "10× Load",
              "5–10 ms",
              "1–2 ms",
              "30 s–5 min",
              "Significant lag; stale reads likely"
            ],
            [
              "Network Partition",
              "1–2 ms (local)",
              ...
              "Divergent replicas; conflict resolution needed"
            ],
            [
              "Partition Recovery",
              "N/A",
              "N/A",
              "1 min–1 hour",
              "Anti-entropy repair; read repair"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            [
              ...
              "10–100 ms",
              "1–2 ms",
              "CP: consensus overhead; AP: no coordination"
            ],
            [
              "Availability (partition)",
              "Unavailable",
              "100% available",
              "CP: sacrifice availability; AP: sacrifice consistency"
            ],
            [
              "Partition Recovery Time",
              "1–10 s",
              "1 ms–1 s",
              "CP: state reconciliation; AP: anti-entropy"
            ],
            [
              "Max Throughput",
              "1K–100K ops/s",
              "100K–1M+ ops/s",
              "AP: no coordination = higher throughput"
            ],
            [
              "Data Divergence Risk",
              "0%",
              "0.1–10%",
              "AP: temporary inconsistency during partition"
            ],
            [
              "Consensus Messages",
              "2N–4N per operation",
              "0–N",
              "CP: Paxos/Raft overhead"
            ],
            [
              "Leader Election Time",
              "100 ms–5 s",
              "N/A",
              "CP: leader-based systems need election"
            ],
            [
              "Network Partitions/Year",
              "1–10",
              "1–10",
              "Same for both; response differs"
            ],
            [
              "Mean Time to Repair",
              "1–10 minutes",
            ...
            [
              ...
              "High",
              "Medium",
              "CP: consensus tuning; AP: conflict resolution"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "Unavailable or slow",
              "Normal + stale reads",
              "CP blocks; AP serves stale data"
            ],
            [
              "5× Load + partition",
              "Unavailable",
              "Normal + conflicts",
              "AP handles load; conflicts increase"
            ],
            [
              "10× Load + partition",
              "Unavailable",
              "Degraded + many conflicts",
              "AP: conflict resolution bottleneck"
            ],
            [
              "Partition heals",
              "1–10 s recovery",
              "1 s–1 min convergence",
              "CP: state sync; AP: anti-entropy"
            ],
            [
              "Normal after heal",
              "Normal",
              "Normal",
              "Both return to steady state"
            ]
          ]
        }
      },
        ...
        table: {
          ...
          rows: [
            ...
            [
              ...
              "CP",
              "etcd, ZooKeeper",
              "Consistency critical for cluster",
              "Kubernetes config"
            ],
            [
              "Real-time analytics",
